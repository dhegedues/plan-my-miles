...
function MileageStats({mileageUnit, currentMileage, setCurrentMileage, minMileage, maxMileage, weeklyAverage, allowedMileagePerDay, remainingWeeks, inputsAreInvalid, setInputsAreInvalid}) {
  ...
  return (
    <div>
      ...
      <div className="mt-2 grid grid-cols-1 xsp:grid-cols-2 sm:grid-cols-3 gap-3 xs:gap-4 sm:gap-5">
        {cards.map((card) => (
          <StatsCard key={card.name} cardData={card} mileageUnit={mileageUnit} setInputsAreInvalid={setInputsAreInvalid} />
        ))}
      </div>
